...
class CommandInterpreter {
    ...
    Table selectClause() {
        ...
        if (_input.nextIf(",")) {
            ...
            if (_input.nextIf("where")) {
                ...
                result = table1.select(table2, columnNames, conditions);
                return result;
            }
            result = table1.select(table2, columnNames, new ArrayList<>());
            return result;
        }
        if (_input.nextIf("where")) {
            List<Condition> conditions = conditionClause(table1);
            result = table1.select(columnNames, conditions);
            return result;
        }
        result = table1.select(columnNames, new ArrayList<>());
        return result;
    ...
    ArrayList<Condition> conditionClause(Table... tables) {
        ...
        Condition newc = condition(tables);
        conditions.add(newc);
        while (_input.nextIs("and")) {
            _input.next("and");
            Condition newc1 = condition(tables);
            conditions.add(newc1);
        }
